...
def get_essay_details():
    ...
    lengths=["300 words", "500 words", "1200 words", "2000 words"]
    ...
    try:
        ...
        length="300 words"
    target_audeince=input("Target audience (e.g, High School Students): ").strip()
    return {"topic": topic, "essay_type": essay_type, "length": length, "target_audience": target_audeince}

def generative_essay_content(details):
    try:
        temp=float(input("Enter temperature (0.1 strucured, 0.7 creative)").strip())
        if not (0.1 <= temp <= 1.0): raise ValueError
    except ValueError:
            print("Invalid temperature, defaulting to 0.3")
            temp=0.3
    ...
    if choice=="1":
        ...
        step_p=f"write step-by-step body arguments for an essay on {details['topic']}. Provide evidence and reasoning."
        body_step= generate_response(step_p, temperature=temp, max_tokens=1024)
        print("\n=== Generated Step-by-Step Body ===\n")
        print(body_step)

    concl_p=f"write a conclcion for an {details['essay_type']} essay about {details['topic']} with the stance of {details['target_audience']}."
...
def run_activity():
    ...
    if not details["topic"] or not details["essay_type"]:
        ...
        return
    generative_essay_content(details)
